...
export async function getTables(salonId: string): Promise<Array<Table>> {
  try {
    const response = await protected_api.get<{ tables: Array<Table> }>(
      `${SALONES}/${salonId}${TABLES}`,
    );
...
export async function createTable(body: TableRequest): Promise<TableResponse> {
  try {
    const response = await protected_api.post<TableResponse>(
      `${SALONES}/${body.salonId}${TABLES}`,
      body,
    );
...
export async function updateTable(body: TableRequest): Promise<TableResponse> {
  try {
    const response = await protected_api.put<TableResponse>(
      `${SALONES}/${body.salonId}${TABLES}/${body.id}`,
      body,
    );
    return response.data;
  } catch (error) {
